...
function QuotaPrompt({ kind, onClose, navigate }) {
  ...
  return <div className="modal-backdrop agent-quota-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="agent-quota-modal" role="dialog" aria-modal="true" aria-labelledby="agent-quota-title">
      ...
      <span>{subscription ? "MEMBERSHIP REQUIRED" : "VIDEO BALANCE NEEDED"}</span>
      <h2 id="agent-quota-title">{subscription ? "开通会员后开始创作" : "当前视频余额为 0"}</h2>
      <p>{subscription ? "免费文字对话需开通古龙引擎包月；付费图片和视频可使用已有余额按次创作。" : "古龙引擎包月权益仍然有效。已审核的包月实付金额会等额计入视频余额；若余额尚未到账，请核对付款审核订单，或充值后继续创作。"}</p>
...
export function WebAgentPage({ user, openAuth, navigate, themeIcon }) {
  ...
  return <main id="main-content" className="web-agent-page">
    ...
    <section className="agent-workspace section-shell">
      <header className="agent-workspace-head"><div className="agent-workspace-intro"><span>PEARAPI FREE MODEL CLOUD</span><h1>今天想完成什么？</h1><p title={`${bootstrap?.models?.length || 8} 个免费模型由古龙服务端统一调度；每次回复展示实时处理节点，不加载第二大脑、本地模型、插件或扩展工作流。`}>{bootstrap?.models?.length || 8} 个免费模型由古龙服务端统一调度；每次回复展示实时处理节点，不加载第二大脑、本地模型、插件或扩展工作流。</p></div><div className="agent-live-status"><i className={bootstrap?.configured ? "ready" : ""} /><span>{loading ? "正在连接" : bootstrap?.configured ? "远程模型已连接" : "等待管理员配置"}</span></div></header>
      ...
      {composerOpen ? <section className={`agent-composer-wrap agent-floating-composer ${isH3Video ? "h3-desktop-composer" : ""}`} role="dialog" aria-modal="false" aria-label="古龙创作输入框">
          ...
          <div className={isH3Video ? "agent-h3-prompt-wrap" : "agent-standard-prompt-wrap"}>
            {isH3Video && <details className="agent-h3-assets-menu">
              <summary title="上传和管理参考素材"><span><UploadSimple size={22} weight="bold" /></span><small>参考素材</small>{attachments.length > 0 && <b>{attachments.length}</b>}</summary>
          ...
          <footer className={isH3Video ? `agent-h3-parameter-rail ${h3VideoMode === "extended" ? "extended" : ""}` : ""}>
