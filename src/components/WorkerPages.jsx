...
function WechatGate({ onSaved, onClose }) {
  ...
  return <div className="modal-backdrop"><form className="worker-wechat-modal" onSubmit={save}><button className="modal-close" type="button" onClick={onClose}><X size={20} /></button><div className="worker-modal-icon"><WechatLogo size={30} weight="fill" /></div><span>CONTACT REQUIRED</span><h2>先填写你的微信号</h2><p>发布需求或接单前必须填写。微信号默认保密，任务另一方只有支付 2 元并通过人工审核后才能查看。</p><label><span>微信号</span><input autoFocus required minLength={5} maxLength={64} value={wechatId} onChange={(event) => setWechatId(event.target.value)} placeholder="请输入你的微信号" /></label>{error && <div className="form-error">{error}</div>}<button className="button primary full" disabled={busy}><ShieldCheck size={19} />{busy ? "正在安全保存" : "保存并继续"}</button></form></div>;
}

function WorkerOfflinePayment({ payment, onPayment, onClose }) {
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  ...
  return <div className="modal-backdrop"><section className="payment-modal offline-payment-modal worker-payment-modal"><button className="modal-close" disabled={busy} onClick={onClose}><X size={20} /></button><div className="payment-logo"><Wallet size={30} /></div><span className="payment-eyebrow">OFFLINE ESCROW</span><h2>{payment.kind === "contact" ? "支付 2 元解锁联系方式" : "支付任务预算，进入托管审核"}</h2><p>{payment.kind === "contact" ? "审核通过后，只向你显示本任务另一方的微信号。" : "预算审核通过后任务才会进入接单大厅；验收通过后系统自动按 80% / 20% 结算。"}</p><img className="payment-qr enterprise-qr" src="/assets/enterprise-payment-qr.jpg" alt="古龙企业微信收款码" /><div className="offline-payment-summary"><span>{payment.kind === "contact" ? "联系方式查看服务" : payment.task?.title || "威客任务预算"}</span><strong>{formatMoney(payment.amountFen)}</strong></div>{error && <div className="form-error">{error}</div>}<div className="payment-dialog-actions"><button className="button secondary" disabled={busy} onClick={onClose}>稍后支付</button><button className="button primary" disabled={busy} onClick={confirm}>{busy ? "正在提交" : "我已支付"}</button></div></section></div>;
}
...
function ContactAction({ task, onPayment }) {
  ...
  return <div className="worker-contact-action">{contact ? <div className="worker-contact-revealed"><WechatLogo size={22} weight="fill" /><span>{contact.displayName}</span><strong>{contact.wechatId}</strong></div> : <button className="button small secondary" disabled={busy} onClick={load}><WechatLogo size={18} />{busy ? "正在读取" : "支付 2 元查看对方微信"}</button>}{error && <small className="worker-inline-error">{error}</small>}</div>;
}
...
export function WorkerPage({ user, openAuth, navigate }) {
  ...
  async function publish(event) {
    ...
    try {
      const result = await apiFetch("/api/worker/tasks", { method: "POST", body: JSON.stringify({ ...form, budgetFen: Math.round(Number(form.budgetYuan) * 100), deadline: new Date(form.deadline).toISOString() }) });
      ...
      setForm({ inputDescription: "", outputDescription: "", exampleDescription: "", deadline: "", budgetYuan: "" }); setInputFiles([]); setOutputFiles([]);
