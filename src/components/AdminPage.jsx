  ...
  return <section className="admin-module pear-token-manager">
    ...
    <div className="pear-token-layout">
      <form className="pear-token-form" onSubmit={save}>
        <div className="pear-token-form-head"><div><LockKey size={29} weight="duotone" /><span><strong>PearAPI API Key 与兼容令牌</strong><small>AES-256-GCM 加密保存，保存后只显示末 4 位</small></span></div><em className={config?.keyConfigured ? "ready" : ""}>{config?.keyConfigured ? "全部模型已连接" : "等待配置 API Key"}</em></div>
        <label><span>PearAPI API Key</span><input type="password" minLength={config?.keyConfigured ? 0 : 8} value={form.key} onChange={(event) => setForm({ ...form, key: event.target.value })} placeholder={config?.keyMasked ? `${config.keyMasked}（留空表示不修改）` : "输入 PearAPI API Key"} /><small>用于文字模型的 Authorization: Bearer 鉴权，以及图片、视频接口的 key 参数；网页版不会回显。</small></label>
        <label><span>PearAPI 兼容令牌（可选）</span><input type="password" minLength={config?.tokenConfigured ? 0 : 8} value={form.token} onChange={(event) => setForm({ ...form, token: event.target.value })} placeholder={config?.tokenMasked ? `${config.tokenMasked}（留空表示不修改）` : "如无旧版兼容需求可留空"} /><small>仅为旧版接入保留；当前网页文字、图片和视频模型均使用上方 API Key。</small></label>
        ...
        <div className="pear-token-actions"><button className="button primary" disabled={Boolean(busy)}><FloppyDisk size={17} />{busy === "save" ? "正在加密保存" : "保存并立即应用"}</button><button className="button secondary" type="button" disabled={Boolean(busy) || !config?.keyConfigured} onClick={testConnection}><Lightning size={17} />{busy === "test" ? "正在验证" : "测试免费模型"}</button></div>
      </form>
      <aside className="pear-token-guide"><span>HOW TO GET API KEY</span><h3>API Key 获取方式</h3><ol><li><b>1</b><span><strong>打开 PearAPI 控制台</strong><small>使用平台账号登录管理后台。</small></span></li><li><b>2</b><span><strong>创建独立 API Key</strong><small>用于古龙官网服务端统一调用文字、图片与视频模型。</small></span></li><li><b>3</b><span><strong>复制并保存到这里</strong><small>建议定期轮换，并只在管理员后台更新。</small></span></li></ol><a className="button secondary full" href={config?.acquisitionUrl || "https://api.pearapi.ai/zh/dashboard"} target="_blank" rel="noreferrer">打开 PearAPI 控制台 <ArrowSquareOut size={17} /></a><a className="pear-doc-link" href={config?.docsUrl || "https://api.pearapi.ai/zh/dashboard/docs"} target="_blank" rel="noreferrer">查看官方接入文档 <ArrowRight size={16} /></a><div className="pear-security-note"><ShieldCheck size={22} weight="duotone" /><p><strong>安全边界</strong><span>浏览器只调用古龙同源接口；API Key 不会进入 HTML、JavaScript、日志或 API 响应。</span></p></div></aside>
    </div>
