...
function releaseProductName(channel) {
  ...
  if (identity.includes("english-coach-portable") || identity.includes("英语教练")) return "英语教练";
  if (identity.includes("short-drama-workbench") || identity.includes("短剧工作台")) return "短剧工作台";
...
function VersionManager() {
  ...
  async function uploadRelease(event) {
    ...
    if (manualUpload.channel.profileKey === "english-coach-portable" && !/\.zip$/i.test(manualUpload.file.name)) {
      setManualUpload((current) => ({ ...current, error: "英语教练绿色版只能上传 ZIP 压缩包。" }));
      return;
    }
...
function PearTokenManager() {
  ...
  return <section className="admin-module pear-token-manager">
    ...
    <div className="pear-free-models"><header><div><span>FREE MODEL ALLOWLIST</span><h3>网页版免费 LLM 白名单</h3></div><strong>{config?.models?.length || 8} 个模型</strong></header><div>{(config?.models || []).map((model) => <article key={model.id}><div><ChatCircleText size={21} weight="duotone" /><span>{model.vendor}</span><em>免费</em></div><h4>{model.name}</h4><code>{model.id}</code><p>{model.description}</p></article>)}</div></div>
