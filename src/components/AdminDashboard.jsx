...
function TrendChart({ trend }) {
  ...
  return <div className="gdx-trend"><div className="gdx-trend-legend"><span><i className="revenue" /> 已确认收入</span><span><i className="active" /> 活跃用户</span><span><i className="register" /> 新增注册</span></div><div className="gdx-trend-plot">{visible.map((row, index) => <div className="gdx-trend-day" key={row.date} title={`${row.date}\n收入 ${formatMoney(row.revenueFen)}\n活跃 ${row.activeUsers || 0}\n注册 ${row.registrations || 0}`}><div className="gdx-user-bars"><i className="active" style={{ height: Number(row.activeUsers || 0) ? `${Math.max(3, Number(row.activeUsers) / userMax * 100)}%` : "0%" }} /><i className="register" style={{ height: Number(row.registrations || 0) ? `${Math.max(3, Number(row.registrations) / userMax * 100)}%` : "0%" }} /></div><i className="gdx-revenue-bar" style={{ height: Number(row.revenueFen || 0) ? `${Math.max(3, Number(row.revenueFen) / revenueMax * 100)}%` : "0%" }} /><span>{index === 0 || index === visible.length - 1 || index % Math.ceil(visible.length / 5) === 0 ? row.date.slice(5) : ""}</span></div>)}</div></div>;
}

function Lifecycle({ rows }) {
  const first = Math.max(1, Number(rows[0]?.value || 0));
  return <div className="gdx-funnel">{rows.map((row, index) => {
    const width = Number(row.value || 0) ? Math.max(8, Number(row.value || 0) / first * 100) : 0;
...
export function AdminDashboard() {
  ...
  return <section className="gdx-dashboard">
    <header className="gdx-hero">
      <div className="gdx-hero-main"><span className="gdx-live"><i /> REAL DATA · LIVE</span><p>今日已确认收入</p><h2>{formatMoney(today.revenueFen)}</h2><div className="gdx-hero-today"><span><strong>{number(today.registrations)}</strong> 新增用户</span><span><strong>{number(today.activeUsers)}</strong> 活跃用户</span><span><strong>{number(today.tasks)}</strong> 智能体任务</span><span><strong>{number(today.brainUploads)}</strong> 第二大脑</span></div><small className="gdx-data-source">仅展示真实业务记录 · {(data.dataSources || []).join(" · ")}</small></div>
    ...
    <section className="gdx-insights"><header><Sparkle size={22} weight="fill" /><div><span>LIVE DATA SUMMARY</span><h3>真实数据经营摘要</h3></div></header><div>{(data.insights || []).length ? (data.insights || []).map((item, index) => <article className={item.tone} key={`${item.title}-${index}`}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{item.title}</strong><p>{item.detail}</p></div><ChartLineUp size={20} /></article>) : <div className="gdx-mini-empty">当前真实业务记录不足以生成摘要</div>}</div></section>
  </section>;
}
