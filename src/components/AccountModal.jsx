      ...
      <section className="account-modal" role="dialog" aria-modal="true" aria-labelledby="account-title">
        ...
        <form onSubmit={submit}>
          ...
          {mode === "forgot" && resetSent && <>
            ...
            <label><span>{recoveryType === "phone" ? "短信验证码" : "邮箱验证码"}</span><div className="input-shell"><Key size={18} /><input required minLength={6} maxLength={6} inputMode="numeric" pattern="[0-9]{6}" value={form.resetCode} onChange={(event) => setForm({ ...form, resetCode: event.target.value.replace(/\D/g, "") })} autoComplete="one-time-code" placeholder={`输入 6 位${recoveryType === "phone" ? "短信" : "邮箱"}验证码`} /></div><small className="reset-code-help">验证码为 6 位数字，10 分钟内有效；重新发送后旧验证码会立即失效。</small></label>
