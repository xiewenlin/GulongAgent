...
- MiniMaxH3 task prompts must use the official three-section Base or six-section Ref2VA order. UI aliases such as `@图片1` compile to angle-bracket labels, multi-image subjects preserve identity only, and task creation always validates the actual queued prompt. The editable Chinese control template is compile-time metadata and must be stripped before dispatch; the formal non-dialogue prompt must be English-only, ambient-only, silent, free of readable text and music, and fail closed when Chinese translation fails. Never add an intermediate-image pipeline.
