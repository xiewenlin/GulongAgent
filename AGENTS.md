...
- MiniMaxH3 task prompts must use the official three-section Base or six-section Ref2VA order. UI aliases such as `@图片1` compile to angle-bracket labels, multi-image subjects preserve identity only, and task creation always validates or deterministically compiles the actual queued prompt; never add spoken content, subtitles, visible titles, or an intermediate-image pipeline.
